...
repo at `…/AgentLens/AgentLens` (remote `origin` →
`https://github.com/Ranger222/AgentLens.git`). It could **not be pushed** during
...
2. Under **Repository permissions**, set **Contents: Read and write** (and ensure
   `Ranger222/AgentLens` is in the token's selected repositories). Save.
3. Make sure `$GITHUB_PAT` in your shell is that token, then:
   ```bash
   cd …/AgentLens/AgentLens
...
- Classic PAT with `repo` scope: `git push https://<TOKEN>@github.com/Ranger222/AgentLens.git build/v1`
- Or add an SSH key to the **Ranger222** account and:
  `git remote set-url origin git@github.com:Ranger222/AgentLens.git && git push -u origin build/v1`
...
gh pr create --base main --head build/v1 \
  --title "AgentLens v1: SDK + dashboard + tests" \
...
The brand and CLI stay **AgentLens** / `agentlens`, but the **PyPI distribution
name `agentlens` is taken** by an unrelated package — choose a different
distribution name (e.g. `lenstrace`) in `pyproject.toml` before `twine upload`.
The import name and `agentlens` command can stay as-is. See
