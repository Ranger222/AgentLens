...
from agentlens.config import Config, resolve_db_path
...
class TestResolveDbPath:
    ...
    def test_default_is_cwd_when_writable(self, tmp_path, monkeypatch):
        monkeypatch.delenv("AGENTLENS_DB", raising=False)
        monkeypatch.chdir(tmp_path)
        assert resolve_db_path() == os.path.join(str(tmp_path), "agentlens.db")
...
class TestConfig:
    def test_from_env_defaults(self, monkeypatch):
        for k in ("AGENTLENS_DISABLED", "AGENTLENS_CAPTURE_IO", "AGENTLENS_MAX_VALUE_LEN"):
    ...
    def test_capture_io_env(self, monkeypatch):
        monkeypatch.setenv("AGENTLENS_CAPTURE_IO", "false")
