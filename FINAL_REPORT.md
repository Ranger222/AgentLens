<!-- AgentLens final report — generated 2026-06-28 -->

# AgentLens — Final Audit & Deployment-Readiness Report

**Date:** 2026-06-28
**Prepared by:** Orchestrated agent-swarm audit + independent runtime verification
**Repo:** `github.com/Ranger222/AgentLens` · branch `build/v1` · commit `55ded72`
...
- **Every raw finding was adversarially verified** by a dedicated skeptic agent that re-read the code and judged each against AgentLens's *local-first, single-user, loopback-by-default* threat model (so severities are realistic, not boilerplate). Result: **18 findings → 18 confirmed/adjusted, 0 rejected**.
...
A wheel was built with the dashboard vendored into `src/agentlens/_webui/`, installed into a **fresh** virtualenv (simulating a real `pip install`), and `agentlens serve` was launched from that install:
...
**Reconciliation of the deployment verdict:** the application *runs and serves correctly in deployment when the wheel is built with the UI vendored* — which the CI `package` job already does. The "deployment ready: NO" verdict below is about **release-process gaps, not the app's runtime ability**: there is no automated/documented vendoring step for a *manual* `python -m build` (a naive build would ship the placeholder), the `.gitignore` protects the wrong path (`sdk/` vs `src/`), and the PyPI distribution name `agentlens` is already taken. These are real and should be cleared before a public release, but the core **capture → store → serve** loop is verified working.

---

## 1. Executive Summary

AgentLens is a well-built, test-covered, single-user developer tool. The code quality is good, the never-raise tracing contract is largely honored, and the architecture (lazy FastAPI import, stdlib-only core, single locked SQLite connection) is sound for its stated local-first model.
...
2. **Wheel vendoring is broken/unverified:** the built dashboard is expected at `src/agentlens/_webui/` (per `pyproject.toml:63` `artifacts = ["src/agentlens/_webui/**"]` and `server/app.py:22`), but that directory **does not exist on disk** — the only build output is at `frontend/dist/`. A wheel built today ships **without the dashboard**, so `agentlens serve` would render the placeholder HTML, not the UI. The frontend build must be vendored into `src/agentlens/_webui/` as part of the release process.
3. **`.gitignore` ignores the wrong path** (`sdk/agentlens/_webui/` vs the real `src/agentlens/_webui/`), so once that build dir is populated for vendoring it is not actually protected from accidental `git add -A` staging of dev-time output.
4. **PyPI name `agentlens`:** `pyproject.toml` declares `name = "agentlens"`. This is a common name and must be confirmed available (or claimed) on PyPI before `twine upload`; verify before release.

Once the high-severity fix lands, the dashboard is correctly vendored into `src/agentlens/_webui/`, and the PyPI name is confirmed, AgentLens is fit to ship as a `0.1.0` alpha.
...
- **Location:** `src/agentlens/server/app.py:62-67`
...
- **Location:** `src/agentlens/cli.py:42` (free-form `--host`), passed verbatim to `uvicorn.run(host=args.host)` at `cli.py:83`/`89`; server has no auth (`server/app.py`).
...
- **Why it matters (local-first):** This is exactly the higher-risk case the threat model carves out. `agentlens serve --host 0.0.0.0` (a common "let me view it from another machine" reflex) immediately publishes the entire unauthenticated trace API — returning captured prompts, completions, and tool args (`models.py:120-121` confirms `input`/`output` fields) — to every host on the LAN, with wildcard CORS on top. No token, no confirmation, no caution in README/help. Severity stays medium (not higher) because the default is safe and this requires deliberate user action — an ergonomic foot-gun, not a default-insecure flaw, and the data is the dev's own traces on their own LAN.
...
- **Location:** `src/agentlens/serialization.py:23-46, 75-99`; README claim at `README.md:100`.
...
- **Why it matters (local-first):** `.git/config` is inherently **untracked** (it lives in `.git/`, never indexed), so it ships nothing to clones and exposes nothing downstream. The configured remote is SSH (`git@github.com:Ranger222/AgentLens.git`), and SSH transport does not invoke credential helpers, so the helper is currently dormant. Residual risk: the helper is unscoped (no host binding / `useHttpPath`), so any **HTTPS** remote git contacts (a typo'd or malicious HTTPS remote) would receive the token. Developer-local hygiene only; token is described as read-only/fine-grained.
...
- **What:** `calculator()` does `return eval(expression)  # noqa: S307 - intentional, for the demo error case`, called only as `calculator("1 / 0")` (line 60) to produce a deliberate `ZeroDivisionError`. `expression` is never derived from network/file/user/LLM input in any shipped path; this is the only `eval()` in the repo. The shipped runtime demo (`src/agentlens/demo.py:42-44`) uses a plain `_ = 1 / 0` with no eval.
...
- **Location:** `src/agentlens/config.py:34-42`, `src/agentlens/storage/sqlite.py:67-71`, `src/agentlens/server/app.py:72`
- **What:** `resolve_db_path()` expands `~`/abspaths from `--db`/`$AGENTLENS_DB` with no allowlist/containment; `SQLiteStorage.__init__` and the server's request-time `storage()` do `os.makedirs(dirname)` then `sqlite3.connect`, auto-creating arbitrary parent dirs.
...
- **Location:** `src/agentlens/server/app.py:69-74`, `src/agentlens/storage/sqlite.py:71`
...
- **What:** `.gitignore` ignores `sdk/agentlens/_webui/`, but no `sdk/` directory exists; the real packaged build location is `src/agentlens/_webui/` (confirmed by `server/app.py:22` `parent.parent / "_webui"` and `pyproject.toml:63` `artifacts = ["src/agentlens/_webui/**"]`). The ignore rule targets a dead path, so a future build emitted into `src/agentlens/_webui/` is **not** actually ignored.
- **Adjustment rationale:** The finding's evidence claimed the bundle is "present and untracked on disk" — this is **false**; `src/agentlens/_webui/` does not currently exist (the only build output is `frontend/dist/`, correctly ignored at line 70). The risk is therefore **latent**, not active: it only materializes once someone wires a build into `src/agentlens/_webui/` (which the release vendoring step requires) and then runs `git add -A`. Worst case is a compiled JS bundle accidentally committed — repo bloat, trivially reverted. Hence low, not medium.
- **Fix:** Change `.gitignore:71` to `src/agentlens/_webui/`. (This intersects with the deployment-readiness vendoring item — see §5.)

#### H-2. Old placeholder name "AgentTrace"/`agenttrace` lingers in docs
- **Location:** `docs/REQUIREMENTS.md:11,19`; `docs/RESEARCH_SYNTHESIS.md:510`
- **What:** The pre-rename placeholder name "AgentTrace" (with `agenttrace serve` and an explicit "(placeholder name—suggest better if you have one)" note) still appears in two design docs; `RESEARCH_SYNTHESIS.md:510` itself flags the drift ("scrub the placeholder name before publish"). Case-insensitive grep confirms the old name is confined to these 3 doc lines — no source, CLI, or packaging uses it.
- **Why it matters:** Cosmetic doc drift that contradicts the shipped `agentlens` name; zero functional/security impact.
- **Fix:** Replace the 3 occurrences with `AgentLens`/`agentlens`, or add a one-line banner marking `REQUIREMENTS.md` as a superseded historical design doc.
...
- **Location:** `src/agentlens/cli.py:67-146`; `src/agentlens/tracer.py:79`; `src/agentlens/server/app.py:38`
...
| B2 | **Dashboard not vendored into the wheel** | Release-blocking (packaging) | `pyproject.toml:63` expects `src/agentlens/_webui/**`; `server/app.py:22` loads from there; directory **does not exist** — only `frontend/dist/` is built | Build the frontend and vendor it into `src/agentlens/_webui/` as part of the release; otherwise `agentlens serve` ships the placeholder HTML, not the UI |
| B3 | **`.gitignore` ignores wrong build path** | Low (hygiene, intersects B2) | `.gitignore:71` = `sdk/agentlens/_webui/`; real path `src/agentlens/_webui/` | Fix to `src/agentlens/_webui/` so dev-time build output is actually ignored and not accidentally staged once B2 populates it |
| B4 | **PyPI name `agentlens` not confirmed available** | Release-blocking (packaging) | `pyproject.toml:6` `name = "agentlens"` | Confirm the name is free/claimable on PyPI before `twine upload`; reserve or rename if taken |
...
- Scrub the "AgentTrace" placeholder from docs (H-2).

The package metadata is otherwise in good shape: correct entry point (`agentlens = agentlens.cli:main`), MIT license declared, alpha classifier, lazy FastAPI deps, py3.9+ matrix. Once B1–B4 are cleared, AgentLens is fit to ship as `0.1.0` alpha.
...
- **Location:** `src/agentlens/decorators.py:94-100` (`async_wrapper`); also `async_gen_wrapper:73-80`; `spans.py:93-97` and `:45-49`
...
- **Location:** `src/agentlens/server/schemas.py:14-34`
...
- **Location:** `src/agentlens/storage/sqlite.py:88-104` (`save_run`) vs schema FK at `:48`
...
- **Location:** `src/agentlens/tracer.py:180-193, :216-218`
...
- **Location:** `src/agentlens/instrument/_common.py:60-78`, called unguarded at `:96` and `:115`
...
- **Location:** `src/agentlens/decorators.py:58-82` (`async_gen_wrapper`), `:104-125` (`gen_wrapper`)
...
- **Location:** `src/agentlens/storage/sqlite.py:236-246` (`_next_seq`), `:66` (`_seq_counters`)
...
2. **Vendor the dashboard into `src/agentlens/_webui/` (B2)** as part of the release build, so the wheel actually ships the UI referenced by `pyproject.toml:63` / `app.py:22`.
3. **Confirm/claim the PyPI name `agentlens` (B4)** before publishing.
4. **Fix `.gitignore:71`** `sdk/agentlens/_webui/` → `src/agentlens/_webui/` (H-1 / B3), so the vendored build dir is correctly ignored during dev once populated.
...
7. **Expand redaction + soften the README claim (S-3):** add missing secret keys, substring matching, a value-shape scrubber, and reword "Secrets-safe" to best-effort/key-based; recommend `AGENTLENS_CAPTURE_IO=0` for sensitive data.
...
16. **Scrub "AgentTrace"/`agenttrace` from docs (H-2).**
...
|---------|----------|-------|-------------------|
| `.gitignore` stale `_webui` path (H-1) | medium | **low** | Risk is latent — `src/agentlens/_webui/` does not exist on disk; finding's "present and untracked" evidence was inaccurate. Worst case is accidental commit of a JS bundle, trivially reverted. |
...
*Report generated for `FINAL_REPORT.md`. Severity judgments reflect AgentLens's local-first, single-user, 127.0.0.1-by-default threat model: browser same-origin bypasses and stated-guarantee gaps are weighted up; "no auth on loopback" is by design and not flagged.*
